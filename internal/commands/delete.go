...
func DeleteCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "delete <queue> <msg_id> [msg_id...]",
...
func runDelete(cmd *cobra.Command, queue string, ids []string) error {
	...
	if len(msgIDs) == 1 {
		var deleted bool
		err = conn.Conn.QueryRow(ctx, "SELECT pgmq.delete($1::text, $2::bigint);", queue, msgIDs[0]).Scan(&deleted)
		if err != nil {
	...
	rows, err := conn.Conn.Query(ctx, "SELECT pgmq.delete($1::text, $2::bigint[]) AS msg_id;", queue, msgIDs)
	if err != nil {
		return dbErrorForQueue(err, queue)
	}
	defer rows.Close()
	...
	if len(deletedIDs) == 0 {
		return errs.NewNotFoundError("no messages found to delete")
	}
	return outputIDs(cmd, deletedIDs)
}
